...
function TemplateEditorModal({ template, onSave, onClose }: {
  ...
  function toggleDay(d: string) { setSchedule(s => ({ ...s, [d]: !s[d] })) }
  ...
  return (
    ...
    >
      <div style={{ background: 'var(--mui-palette-background-paper)', borderRadius: 16, boxShadow: '0 24px 80px rgba(0,0,0,.25)', width: '100%', maxWidth: 820, maxHeight: '90vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {/* Header */}
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--mui-palette-divider)', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', flex: 1, overflow: 'hidden' }}>
          ...
          <div style={{ padding: '20px 24px', overflowY: 'auto', borderRight: '1px solid var(--mui-palette-divider)', display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {BLOCK_COLORS.map(c => (
                  <div key={c} onClick={() => setColor(c)} style={{ width: 22, height: 22, borderRadius: '50%', background: c, cursor: 'pointer', border: `2.5px solid ${color === c ? 'var(--mui-palette-text-primary)' : 'transparent'}`, transition: 'border 150ms' }} />
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {DAYS_SHORT.map(d => (
                  <button key={d} onClick={() => toggleDay(d)} style={{ padding: '5px 12px', borderRadius: 8, border: `1.5px solid ${schedule[d] ? color : 'var(--mui-palette-divider)'}`, background: schedule[d] ? `${color}18` : 'transparent', color: schedule[d] ? color : 'var(--mui-palette-text-disabled)', fontSize: '.8rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 150ms' }}>
                    {DAYS_LABEL[d]}
                  </button>
                ))}
              </div>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                ...
                <button onClick={addBlock} style={{ background: 'var(--mui-palette-primary-main)', color: '#fff', border: 'none', borderRadius: 7, padding: '5px 12px', fontSize: '.75rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, fontFamily: 'inherit' }}>
                ...
                {sorted.map(bl => (
                  <div key={bl.id} style={{ background: 'var(--mui-palette-action-hover)', borderRadius: 10, overflow: 'hidden', border: `1.5px solid ${editing === bl.id ? bl.color : 'transparent'}`, transition: 'border 150ms' }}>
                    <div onClick={() => setEditing(editing === bl.id ? null : bl.id)} style={{ padding: '10px 12px', display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}>
                      ...
                      <span style={{ fontSize: '.65rem', fontWeight: 700, background: `${bl.color}18`, color: bl.color, padding: '2px 7px', borderRadius: 9999 }}>
                        {bl.type === 'free' ? 'Free' : 'Fixed'}
                      </span>
                      <button onClick={e => { e.stopPropagation(); removeBlock(bl.id) }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--mui-palette-text-disabled)', fontSize: 14, lineHeight: 1, padding: 0 }}>×</button>
                    </div>
                    {editing === bl.id && (
                      <div style={{ padding: '0 12px 14px', display: 'flex', flexDirection: 'column', gap: 10, borderTop: '1px solid var(--mui-palette-divider)' }} onClick={e => e.stopPropagation()}>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 10 }}>
                          <div>
                            ...
                            <input value={bl.label} onChange={e => updateBlock(bl.id, 'label', e.target.value)}
                              style={{ width: '100%', border: '1px solid var(--mui-palette-divider)', borderRadius: 7, padding: '7px 10px', fontSize: '.8125rem', background: 'var(--mui-palette-background-paper)', color: 'var(--mui-palette-text-primary)', outline: 'none', fontFamily: 'inherit' }} />
                          </div>
                          <div>
                            ...
                            <div style={{ display: 'flex', gap: 6 }}>
                              {(['fixed', 'free'] as const).map(t => (
                                <button key={t} onClick={() => updateBlock(bl.id, 'type', t)} style={{ flex: 1, padding: '7px', borderRadius: 7, border: `1.5px solid ${bl.type === t ? bl.color : 'var(--mui-palette-divider)'}`, background: bl.type === t ? `${bl.color}18` : 'transparent', color: bl.type === t ? bl.color : 'var(--mui-palette-text-disabled)', fontSize: '.75rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', textTransform: 'capitalize' }}>{t}</button>
                              ))}
                            </div>
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div style={{ display: 'flex', gap: 7 }}>
                            {BLOCK_COLORS.map(c => (
                              <div key={c} onClick={() => updateBlock(bl.id, 'color', c)} style={{ width: 18, height: 18, borderRadius: '50%', background: c, cursor: 'pointer', border: `2px solid ${bl.color === c ? 'var(--mui-palette-text-primary)' : 'transparent'}` }} />
                            ))}
                          </div>
                        </div>
                      </div>
          ...
          {/* Right — preview */}
          <div style={{ overflowY: 'auto', padding: '20px', background: 'var(--mui-palette-action-hover)' }}>
        ...
        <div style={{ padding: '16px 24px', borderTop: '1px solid var(--mui-palette-divider)', display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <button onClick={onClose} style={{ background: 'none', border: '1.5px solid var(--mui-palette-divider)', borderRadius: 9, padding: '9px 20px', fontSize: '.875rem', fontWeight: 600, color: 'var(--mui-palette-text-secondary)', cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>
          <button onClick={() => onSave({ id: template?.id ?? uid(), name, color, schedule, blocks })}
            style={{ background: 'var(--mui-palette-primary-main)', color: '#fff', border: 'none', borderRadius: 9, padding: '9px 24px', fontSize: '.875rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 2px 8px rgba(201,124,74,.3)' }}>
            Save template
          </button>
        </div>
      </div>
...
function TemplatesPanel({ templates, activeTemplateId, onApply, onEdit, onAdd, onDelete, onClose }: {
  ...
  return (
    <div style={{ width: 300, background: 'var(--mui-palette-background-paper)', borderRadius: 14, boxShadow: '0 3px 12px rgba(47,43,61,.14)', display: 'flex', flexDirection: 'column', overflow: 'hidden', border: '1px solid var(--mui-palette-divider)' }}>
      <div style={{ padding: '16px 18px', borderBottom: '1px solid var(--mui-palette-divider)', display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--mui-palette-text-disabled)', fontSize: 18, lineHeight: 1 }}>×</button>
      </div>
            ...
            <div key={t.id} style={{ marginBottom: 10 }}>
              <div style={{ background: isActive ? `${t.color}10` : 'var(--mui-palette-action-hover)', borderRadius: 12, border: `1.5px solid ${isActive ? t.color : 'var(--mui-palette-divider)'}`, overflow: 'hidden', transition: 'all 200ms' }}>
                <div style={{ padding: '12px 14px' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={() => onApply(isActive ? null : t.id)} style={{ flex: 1, padding: '7px 10px', borderRadius: 8, background: isActive ? t.color : `${t.color}18`, color: isActive ? '#fff' : t.color, border: `1.5px solid ${t.color}`, fontSize: '.75rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 180ms' }}>
                      {isActive ? '✓ Applied' : 'Apply today'}
                    </button>
                    <button onClick={() => onEdit(t)} style={{ background: 'var(--mui-palette-action-hover)', border: '1px solid var(--mui-palette-divider)', borderRadius: 8, padding: '7px 10px', fontSize: '.75rem', color: 'var(--mui-palette-text-secondary)', cursor: 'pointer', fontFamily: 'inherit' }}>
                      <i className='tabler-edit' style={{ fontSize: 13 }} />
                    </button>
                    <button onClick={() => onDelete(t.id)} style={{ background: 'rgba(255,76,81,.08)', border: '1px solid rgba(255,76,81,.2)', borderRadius: 8, padding: '7px 10px', fontSize: '.75rem', color: '#FF4C51', cursor: 'pointer', fontFamily: 'inherit' }}>
      ...
      <div style={{ padding: '12px', borderTop: '1px solid var(--mui-palette-divider)' }}>
        <button onClick={onAdd} style={{ width: '100%', background: 'var(--mui-palette-primary-main)', color: '#fff', border: 'none', borderRadius: 9, padding: '10px', fontSize: '.875rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, fontFamily: 'inherit', boxShadow: '0 2px 8px rgba(201,124,74,.25)' }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        {activeTemplate ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, background: `${activeTemplate.color}12`, border: `1px solid ${activeTemplate.color}30`, borderRadius: 8, padding: '6px 12px' }}>
            <div style={{ width: 7, height: 7, borderRadius: '50%', background: activeTemplate.color }} />
            <span style={{ fontSize: '.8rem', fontWeight: 600, color: activeTemplate.color }}>{activeTemplate.name}</span>
            <span style={{ fontSize: '.7rem', color: 'var(--mui-palette-text-disabled)' }}>active</span>
          </div>
        ) : (
          <div style={{ fontSize: '.8rem', color: 'var(--mui-palette-text-disabled)', fontStyle: 'italic' }}>No template applied</div>
        )}
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
          <button onClick={() => setShowPanel(p => !p)} style={{ background: showPanel ? 'var(--mui-palette-primary-main)' : 'var(--mui-palette-background-paper)', color: showPanel ? '#fff' : 'var(--mui-palette-text-secondary)', border: `1.5px solid ${showPanel ? 'var(--mui-palette-primary-main)' : 'var(--mui-palette-divider)'}`, borderRadius: 9, padding: '8px 16px', fontSize: '.8125rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7, fontFamily: 'inherit', boxShadow: '0 1px 4px rgba(47,43,61,.1)', transition: 'all 180ms' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: showPanel ? '240px 1fr 300px' : '240px 1fr', gap: 14, alignItems: 'start' }}>
        ...
        <div style={{ background: 'var(--mui-palette-background-paper)', borderRadius: 14, boxShadow: '0 3px 12px rgba(47,43,61,.14)', overflow: 'hidden' }}>
          <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--mui-palette-divider)', display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ marginLeft: 'auto', fontSize: '.75rem', color: 'var(--mui-palette-text-disabled)' }}>
          ...
          {/* Footer */}
          <div style={{ padding: '10px 18px', borderTop: '1px solid var(--mui-palette-divider)', display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#FF4C51' }} />
            ...
            <span style={{ marginLeft: 'auto', fontSize: '.7rem', color: 'var(--mui-palette-text-disabled)' }}>Drag tasks to schedule</span>
          </div>
        </div>
